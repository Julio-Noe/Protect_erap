...
fc=open('corpus_protect/2/in_corpus.txt', 'a')
iss=open('corpus_protect/2/issues_AIAAIC.txt', 'w')
gold=open('corpus_protect/2/paragraph_issue.txt', 'a')
...
file=open('corpus_protect/2/corpus_final.csv', 'w', newline='')
...
for url_m in matrix_repo:
    ...
    url=ur.decode('utf-8')
    fr = open('corpus_protect/2/in_corpus.txt','r')#aqui se guardan cada una de la informacion
    filer=fr.readlines()
    if(url+'\n' in filer):
        pass
    else:
        if((url not in no) ):
            ...
            for url2 in list_repo2:
                ...
                file1=open('corpus_protect/2/corpus'+str(n_url)+'_'+str(n)+'.txt', 'a')
